...
const signToken = userID => {
    return JWT.sign({
        iss: "<your code>",
        sub: userID
    }, "<your code>", { expiresIn: "1h" }); // have to match the code in passport.js
}
...
userRouter.get('/admin', passport.authenticate('jwt', { session: false }), (req, res) => {
    if (req.user.role === 'admin') {
        res.status(200).json({ message: { msg: 'You are an admin', msgError: false } });
    }
    else
        res.status(403).json({ message: { msg: "You're not an admin", msgError: true } });
});
